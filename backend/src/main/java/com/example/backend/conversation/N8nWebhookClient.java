...
public class N8nWebhookClient {
    ...
    public Map<String, Object> ask(String question, List<String> agents, Integer size) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                ...
                throw new AppException(ErrorCode.N8N_WEBHOOK_REQUEST_FAILED);
            }
            return objectMapper.readValue(response.body(), new TypeReference<Map<String, Object>>() {});
        } catch (InterruptedException error) {
            Thread.currentThread().interrupt();
            throw new AppException(ErrorCode.N8N_WEBHOOK_REQUEST_INTERRUPTED, error);
        } catch (JsonProcessingException error) {
            throw new AppException(ErrorCode.N8N_WEBHOOK_RESPONSE_PARSE_FAILED, error);
        } catch (IOException error) {
            throw new AppException(ErrorCode.N8N_WEBHOOK_CALL_FAILED, error);
        }
    }
    ...
    private String requestBody(String question, List<String> agents, Integer size) {
        ...
        try {
            ...
            throw new AppException(ErrorCode.N8N_WEBHOOK_REQUEST_SERIALIZE_FAILED, error);
        }
    }
